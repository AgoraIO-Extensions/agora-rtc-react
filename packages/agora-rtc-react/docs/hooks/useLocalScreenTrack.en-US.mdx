...
- If you get the video track and audio track objects from the return value but only publish one track, you need to manually close the unpublished track using the Web SDK's [`ILocalTrack.close`](https://api-ref.agora.io/en/video-sdk/reactjs/2.x//ILocalTrack.close.html) method before stopping screen sharing. The other published track will be closed automatically.
